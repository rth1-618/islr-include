...
def process_landmarks(landmarks):
    ...
    if landmarks is not None:
        keypoints = protobuf_to_dict(landmarks)
    ##
    # keypoints = []
    ...
    #                          })
    ##
    # print(f"KEYPOINTS::::::: {keypoints['landmark']}")
...
def process_hand_keypoints(results):
    ...
    if results.multi_hand_landmarks is not None:
        if len(results.multi_hand_landmarks) > 0:
            ...
            hand1_x, hand1_y = process_hand_landmarks(hand1)

        if len(results.multi_hand_landmarks) > 1:
            hand2 = results.multi_hand_landmarks[1]
            hand2_x, hand2_y = process_hand_landmarks(hand2)
...
def process_video(path, save_dir):
    ...
    if not os.path.isfile(path):
        ...
        replacement = "MP4"  # replace with this
    ...
    while cap.isOpened():
        ...
        # print("POSE RESULTS", (pose_results.pose_landmarks))
        # print(f"\n\nPOSE obj:::: ----- {vars(hand_results)}\n\n")
        hand1_x, hand1_y, hand2_x, hand2_y = process_hand_keypoints(
            hand_results)
        pose_x, pose_y = process_pose_keypoints(pose_results)
        # if len(pose_x) == 0:
        #     print("POSE RESULTS", len(pose_x))

        if len(pose_x) != 0 and len(pose_y) != 0:
        ...
        pose_x = pose_x if len(pose_x) != 0 else [np.nan] * 33
        pose_y = pose_y if len(pose_y) != 0 else [np.nan] * 33
    ...
    if np.shape(pose_points_x)[0] == 1:
        print("SHAPE OF POSE POINTS: ", np.shape(pose_points_x))
    save_data = {
        "uid": uid,
        "label": label,
...
def load_train_test_val_paths(args):
    ...
    val_paths = load_file(
        f"train_test_paths/{args.dataset}_val.txt", args.include_dir)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate keypoints from Mediapipe")
    parser.add_argument(
        "--include_dir",
        default="",
        type=str,
